...
def run_search(
    ...
    best_outcome: EvalOutcome | None = None
    best_score = 0.0

    for round_idx in range(config.max_rounds):
        if best_outcome:
            metrics = best_outcome.result.get_metrics()
        ...
        if score > best_score:
            best_outcome = EvalOutcome(impl=impl, result=result)
            best_score = score
    ...
    return SearchResult(
        ...
        score=best_score,
        result=best_outcome.result if best_outcome else None,
